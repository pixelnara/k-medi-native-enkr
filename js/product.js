...
(function () {
  ...
  var allProds = [];
  try {
    allProds = JSON.parse(sessionStorage.getItem("cosm_products")) || [];
  } catch (e) {}
  var recProds = allProds
    ...
    .slice(0, 3);
  var recImgs = recProds.map(function (r) {
    return r.img;
  });
  ...
  var extraPool = allProds
    .filter(function (x) {
      return x.id !== p.id && recImgs.indexOf(x.img) === -1;
    })
    .map(function (x) {
      return x.img;
    });
  var pIdx = allProds.findIndex
    ? allProds.findIndex(function (x) {
        return x.id === p.id;
      })
    : 0;
  ...
  document.getElementById("pdPrev").addEventListener("click", function () {
    setImg(curIdx - 1);
  });
  document.getElementById("pdNext").addEventListener("click", function () {
    setImg(curIdx + 1);
  });
  setImg(0);
  ...
  var featureItems = features
    .map(function (f) {
      return (
        ...
        '<span class="pd-feat-label"><span class="pd-feat-icon">' +
        f.icon +
        "</span>" +
        '<strong class="pd-feat-title">' +
        f.title +
        "</strong></span>" +
        '<p class="pd-feat-desc">' +
        f.desc +
        "</p></div>"
      );
    })
  ...
  ph.innerHTML =
    ...
    '<p class="pd-detail-hero__label">' +
    (p.category || "") +
    "</p>" +
    '<h3 class="pd-detail-hero__title">' +
    p.name +
    "</h3>" +
    '<p class="pd-detail-hero__desc">' +
    (p.desc || "피부 깊은 곳부터 채워주는 프리미엄 스킨케어") +
    "</p>" +
    "</div>" +
    '<div class="pd-detail-features">' +
    featureItems +
    "</div>" +
